...
node_name=`hostname |grep -io node[0-9] |awk '{print tolower($0)}'`
...
cmd="$KAFKA_HOME/bin/kafka-server-start.sh $KAFKA_HOME/config/server-$node_name.properties > $broker_log_file 2>&1"
echo "$cmd"
eval "$cmd" &
