...
main_class_name="com.cleverfishsoftware.kafkaproxied.kafkaconsumer.KafkaConsumerRunner"
cmd="java -cp .:target/KafkaConsumer-1.0-SNAPSHOT.jar $main_class_name $bootstrap_server $consumer_group_id $consumer_id $topic $sleep $threads $verbose $report_interval $ssl_true $keystore_file $keystore_password"
display_command "$cmd"
